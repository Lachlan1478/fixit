...
# ── Sensitive-file blocklist (dotfiles + .db) ─────────────────────────────────

@pytest.mark.api
async def test_file_dotfile_blocked(client):
    """.env inside the workspace → 403 (dotfile blocklist)."""
    response = await client.get("/file?path=claude-agent/.env")
    assert response.status_code == 403


@pytest.mark.api
async def test_file_dot_directory_blocked(client):
    """Any path with a dot-directory component (.git) → 403."""
    response = await client.get("/file?path=.git/config")
    assert response.status_code == 403


@pytest.mark.api
async def test_file_nested_dot_component_blocked(client):
    """Dot component in the middle of the path → 403."""
    response = await client.get("/file?path=claude-agent/.claude/settings.json")
    assert response.status_code == 403


@pytest.mark.api
async def test_file_db_extension_blocked(client):
    """.db files (SQLite) → 403 even when non-hidden."""
    response = await client.get("/file?path=orchestrator/sessions.db")
    assert response.status_code == 403


@pytest.mark.api
async def test_file_db_extension_case_insensitive(client):
    """Extension check is case-insensitive → .DB also 403."""
    response = await client.get("/file?path=whatever/data.DB")
    assert response.status_code == 403


@pytest.mark.api
async def test_serve_dotfile_blocked(client):
    """/serve applies the same dotfile blocklist → 403."""
    response = await client.get("/serve?path=claude-agent/.env")
    assert response.status_code == 403


@pytest.mark.api
async def test_serve_db_blocked(client):
    response = await client.get("/serve?path=orchestrator/sessions.db")
    assert response.status_code == 403


@pytest.mark.api
async def test_image_dot_directory_blocked(client):
    """/image inside a dot-directory → 403."""
    response = await client.get("/image?path=.git/logo.png")
    assert response.status_code == 403


@pytest.mark.api
async def test_file_normal_path_still_served(client):
    """Sanity: a regular non-hidden file is unaffected by the blocklist."""
    response = await client.get("/file?path=claude-agent/server.py")
    assert response.status_code == 200
