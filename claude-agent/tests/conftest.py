...
def reset_claude_session():
    """Clear in-memory session + history + locks between tests.

    Locks must be cleared because asyncio.Lock binds to the event loop it is
    first acquired on, and each test gets a fresh loop.
    """
    import claude_session as cs

    cs._agent_sessions.clear()
    cs._conversation_history.clear()
    cs._agent_locks.clear()
    yield
    cs._agent_sessions.clear()
    cs._conversation_history.clear()
    cs._agent_locks.clear()
...
# ── Auth isolation (autouse) ──────────────────────────────────────────────────

@pytest.fixture(autouse=True)
def no_api_key(monkeypatch):
    """Ensure AGENT_API_KEY from the environment/.env never leaks into tests."""
